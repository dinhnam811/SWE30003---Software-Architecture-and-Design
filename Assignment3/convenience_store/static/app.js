...
async function loadCart() {
  try {
    ...
    if (data.items.length === 0) {
      ...
      if (checkoutBtn) {
        const canCheckout =
          data.can_checkout !== undefined ? data.can_checkout : data.items.every((i) => i.stock_ok !== false)
...
async function updateCartItem(productId, quantity) {
  try {
    ...
    const formData = new FormData()
    formData.append("product_id", productId)
    formData.append("quantity", Number.parseInt(quantity))
  ...
  receiptContent.innerHTML = `
        <div class="receipt">
            ...
            `,
                    ...
                    .join("")
                : '<p style="padding-left: 20px;">No items</p>'
            }
...
async function updateProduct(e, productId) {
  ...
  formData.append("price", Number.parseFloat(form.price.value))
  formData.append("stock", Number.parseInt(form.stock.value))
...
async function updateOrderStatus(orderId, status) {
  try {
    const response = await fetch(
      `${API_BASE}/api/admin/orders/${orderId}/status?session_id=${sessionId}&status=${status}`,
      {
        method: "PUT",
      },
    )
